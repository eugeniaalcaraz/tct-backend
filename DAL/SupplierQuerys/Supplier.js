...
function getSupplierPeopleCertifications(supplierId) {
  let stringQuery = `SELECT 
                        SP.ID_SUPPLIER idSupplier,
                        PC.ID idCertification,
                        PC.DESCRIPTION description,
                        PC.TYPE type,
                        PC.SUBCATEGORY subcat,
                        PC.COMMENT_GENERIC_PLANET commentGenericPlanet,
                        PC.COMMENT_GENERIC_QUIMICALS commentGenericQuimicals,
                        PC.COMMENT_GENERIC_MATERIALS commentGenericMaterials,
                        PC.COMMENT_GENERIC_PEOPLE commentGenericPeople
                        FROM SUPPLIER_CERTIFICATIONS SP 
                        INNER JOIN PEOPLE_CERTIFICATION PC ON PC.ID = SP.CERTIFICATION_ID
                        WHERE  SP.CERTIFICATION_TYPE = 2 AND SP.ID_SUPPLIER IN (${supplierId})`;
  return new Promise(function (resolve, reject) {
    con.query(stringQuery, function (err, rows, fields) {
      if (err) {
        return reject(err);
...
function getSupplierPlanetCertifications(supplierId) {
  let stringQuery = `SELECT 
                        SP.ID_SUPPLIER idSupplier,
                        PC.ID idCertification,
                        PC.DESCRIPTION description,
                        ...
                        PC.COMMENT_GENERIC_PEOPLE commentGenericPeople
                        FROM SUPPLIER_CERTIFICATIONS SP 
                        INNER JOIN PLANET_CERTIFICATION PC ON PC.ID = SP.CERTIFICATION_ID
                        WHERE SP.CERTIFICATION_TYPE = 1 AND SP.ID_SUPPLIER IN (${supplierId})`;
  return new Promise(function (resolve, reject) {
    con.query(stringQuery, function (err, rows, fields) {
      if (err) {
        return reject(err);
...
function getSupplierProcessCertifications(supplierId) {
  let stringQuery = `SELECT 
                        SP.ID_SUPPLIER idSupplier,
                        PC.ID idCertification,
                        PC.DESCRIPTION description,
                        PC.TYPE type
                        FROM SUPPLIER_CERTIFICATIONS SP 
                        INNER JOIN PROCESS_CERTIFICATION PC ON PC.ID = SP.CERTIFICATION_ID
                        WHERE  SP.CERTIFICATION_TYPE = 3 AND SP.ID_SUPPLIER IN (${supplierId})`;
  return new Promise(function (resolve, reject) {
    con.query(stringQuery, function (err, rows, fields) {
      if (err) {
        return reject(err);
...
function saveSupplierCertifications(
  supplierId,
  certificationId,
  certificationType,
...
function updateSupplier(data, performance) {
  ...
  if (
    data.commercialRelationDate !== undefined &&
    data.commercialRelationDate !== null
  ) {
    if (!dataAdded) {
      stringQuery +=
        " COMMERCIAL_RELATION_DATE = " +
        getFormattedDate(data.commercialRelationDate);
    } else {
      stringQuery +=
        ", COMMERCIAL_RELATION_DATE = " +
        getFormattedDate(data.commercialRelationDate);
  ...
  if (
    data.estimatedAnualOrder !== undefined &&
    data.estimatedAnualOrder !== null
  ) {
    if (!dataAdded) {
...
function getProductTypesForSuppliersId(supplierId, product) {
  let stringQuery = ` SELECT 
    ...
    if(product !== undefined){
      stringQuery += `AND SP.ID IN (${product})`
    }
  return new Promise(function (resolve, reject) {
    con.query(stringQuery, function (err, rows, fields) {
      if (err) {
...
function getSupplierForFilter(idMerchant, score, alias, origin,type) {
  let stringQuery = `
    ...
    S.COMMERCIAL_RELATION_DATE relationShipDate,
    S.HAS_ANUAL_CONTRACT hasAnualContract,
    ST.RELATIONSHIP_TYPE relationshipType
  ...
  if (score !== undefined) {
    stringQuery += ` AND UPPER(S.PERFORMANCE) = '${score.toUpperCase()}'`;
  }
  if (alias !== undefined) {
    stringQuery += ` AND UPPER(S.ALIAS) = '${alias.toUpperCase()}'`;
  }
  if (origin !== undefined) {
    stringQuery += ` AND S.ID_COUNTRY = ${origin}`;
  }
  if (type !== undefined) {
    stringQuery += ` AND S.SUPPLIER_TYPE_ID = ${type}`;
  }
  return new Promise(function (resolve, reject) {
    con.query(stringQuery, function (err, rows, fields) {
      if (err) {
...
module.exports.getSupplierPeopleCertifications =
  getSupplierPeopleCertifications;
module.exports.getSupplierPlanetCertifications =
  getSupplierPlanetCertifications;
...
module.exports.getSupplierProcessCertifications =
  getSupplierProcessCertifications;
module.exports.getSupplierPeopleCertifications =
  getSupplierPeopleCertifications;
module.exports.getSupplierPlanetCertifications =
  getSupplierPlanetCertifications;
